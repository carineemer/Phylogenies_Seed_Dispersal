...
author: "Carine Emer y Miguel Verdú"
date: "27/04/2018"
...
```{r input data}
setwd("~/Dropbox/Phylogenetic interactions/Phylogenies - Metanetwork Miguel/data")
...
#We are working with the regional pool of species to estimate their ED. CHECK THIS!!!!! 
...
#setwd("~/Dropbox/Phylogenetic interactions/Phylogenies Atlantic Forest")
...
ED100_p2<-do.call(cbind.data.frame,ED100_p)
ED100_p2<-ED100_p2[,-seq (3, 200, 2),] ## exclude columns with species names but first one
rownames(ED100_p2)<-ED100_p2[,1] 
ED100_p2<-ED100_p2[,-1] ## exclude first column
colnames(ED100_p2)<-paste("tree",1:100,sep="") ## give name to each column
ED100_p <- cbind(Plant_Species2 = rownames(ED100_p2), ED100_p2) # give name to first column to later match with other df
...
###-------------------------- Body mass - 223 species at the regional level

## getting unique bird species and mergind with their body mass (from Atlantic filtered as 'data', natives and mutualistic)
...
bm_birds1<-merge(birds_unique, data,by="Frugivore_Species2")
bm_birds<-bm_birds1[,c("Frugivore_Species2", "Frug_Body_Mass", "Frugivore_Species", "interaction","Plant_Species2","seed_diameter", "Plant_Species")]
...
###
### -------------------------Seed diameter - 441 species at the regional level
...
#quitar<-setdiff((sd_unique$Plant_Species2),(ED100_p2$Plant_Species2))
...
#### ------------------- BIRDS - 187 sp among the selected communities - 223 regional level

bm_ED_commu<-join(bm_commu, ED100, by="Frugivore_Species2") 
bm_unique_commu<-subset(bm_commu,!duplicated(bm_commu[,1])) # using unique spp for the traits correlations
...
####
##-------------------------PLANTS - 361 spp among the selected communities
#names(ED100_p)[names(ED100_p) == 'Plant_Species'] <- 'Plant_Species'

sd_ED_commu<-join(sd_commu, ED100_p, by="Plant_Species2") 
sd_unique_commu<-subset(sd_commu,!duplicated(sd_commu[,1])) # using unique spp for the traits correlations
row.names(sd_unique_commu)<-sd_unique_commu$Plant_Species
sd_unique_commu2<-join(sd_unique_commu, ED100_p, by = "Plant_Species2", type = "left")
...
### ----------- regional level
results_ed_bm<-corPerm3(log(bm_unique_atl$Frug_Body_Mass),ED100$mean_birds_ed,nperm=999, tail=2)
results_ed_bm

### ----------- community level
results_ed_bm_commu<-corPerm3(log(bm_unique_commu2$Frug_Body_Mass),bm_unique_commu2$mean_birds_ed,nperm=999, tail=2)
results_ed_bm_commu


par(mfrow=c(1,2))
plot(log(bm_unique_atl$Frug_Body_Mass)~log(bm_unique_atl$mean_birds_ed),
     cex.main=.7,cex.lab=.8,main = "Regional Level",
     ylab="Body mass (g)",
     xlab="Evolutionary Distinctiveness (log[mean ED])"
     )
    #dev.copy2pdf(file="Figures/ED_bodymass_commu.pdf")

plot(log(bm_unique_commu2$Frug_Body_Mass)~log(bm_unique_commu2$mean_birds_ed),
...
##### ----- community level  ### 361 species
seed_diam1<-as.numeric(sd_unique_commu2$seed_diameter)
results_ed_sd_commu<-corPerm3(log(seed_diam1),log(sd_unique_commu2$mean_plants_ed),nperm=999, tail=2)
results_ed_sd_commu


par(mfrow=c(1,2))
plot(log(seed_diam)~log(ED100_p$mean_plants_ed),
     cex.main=.7,cex.lab=1,main = "Regional Level",
     ylab="Seed diameter (mm)",
     xlab="Evolutionary Distinctiveness (log[mean ED])"
     )
    #dev.copy2pdf(file="Figures/ED_sd_atlantic.pdf")

plot(log(seed_diam1)~log(sd_unique_commu2$mean_plants_ed),
...
### alternative for the loop, by Carol Bello
#library(plyr)
sum_bm<-NULL
...
mean_bm<-NULL
mean_body2=ddply(bm_commu, .(site), summarize, mean_bm=mean(Frug_Body_Mass, na.rm=T))
mean_body2
...
mean_body2[,3]<-NULL
mean_body2$def_mean<-(1-(mean_body2$mean_bm)/mean_pool)
mean_body2[,4]<-NULL
mean_body2$def_sum<-(1-(sum_bm_site$sum_bm)/sum_pool)

communities1<-with(communities, communities[order(site) , ])
mean_body2<-with(mean_body2, mean_body2[order(site) , ])
new_commu<-cbind(communities1, mean_body2)
new_commu3<-merge(communities1,mean_body2, by= "site", all=T)

def_site<-cbind.data.frame((new_commu3$site),new_commu3$def_mean,new_commu3$def_sum, new_commu3$area_ha)

###### 
##plots - add the number of each within the circles
plot(def_site$`new_commu3$def_sum`~log((def_site$`new_commu3$area_ha`)),
     xlab = "Area of the fragments (log[ha])",
     ylab = "Defaunation index",
     cex.lab=1.3, pch = 19, col = "darkgreen", cex = 2
     )
      
cor(new_commu3$def_sum,log(new_commu3$area_ha))
#[1] -0.9333154 - not as correlated as thought
m1<-lm(new_commu3$def_sum~log(new_commu3$area_ha))
summary(m1)
abline(m1)
...
#### order dataframes by interaction - 3566 levels of interactions, but 3502 observations

df<-int_commu[with(int_commu,order(interaction)),]
df1<-sd_ED_commu[with(sd_ED_commu,order(interaction)),]
df1<-df1[,c(1:8,109)]


#### calculating the mean ED per interactions

int_commu1<-join(df,df1, by = "Plant_Species", type = "left", match = "first")
int_commu1<-int_commu1[,c(1:4,10,12)]
int_commu2<-join(int_commu1,bm_ED_commu, by = "Frugivore_Species", type = "left", match = "first")
int_commu2<-int_commu2[,c(1:8,13)]
int_commu2[,10]<-NULL
int_commu2$int_ed<-with(int_commu2, mean_birds_ed+mean_plants_ed)

####### calculating mean of interactions ED per site
mean_ED_site<-NULL
int_site=ddply(int_commu2, .(site), summarize, mean_ED_site=mean(int_ed, na.rm=TRUE))
int_site

sum_ED_site<-NULL
int_site1=ddply(int_commu2, .(site), summarize, sum_ED_site=sum(int_ed, na.rm=T))
int_site1

int_ed_site<-cbind(int_site, int_site1)
int_ed_site<-cbind(int_ed_site, def_site)
int_ed_site1 <- int_ed_site[,-(3),drop=TRUE]
int_ed_site1 <- int_ed_site1[,-(4),drop=T]
colnames(int_ed_site1)[colnames(int_ed_site1) == 'new_commu3$def_mean'] <- 'def_mean'
colnames(int_ed_site1)[colnames(int_ed_site1) == 'new_commu3$def_sum'] <- 'def_sum'
colnames(int_ed_site1)[colnames(int_ed_site1) == 'new_commu3$area_ha'] <- 'area_ha'
#ed_commu<- int_ed_site1[,-(7:10),drop=T]

par(mfrow=c(1,2))
#plot(int_ed_site1$mean_ED_site~int_ed_site1$def_mean)
plot(log(int_ed_site1$sum_ED_site)~int_ed_site1$def_sum,
     xlab = "Defaunation",
     ylab = "Sum of Evolutionary Distinctiveness of Interactions")
plot(log(int_ed_site1$sum_ED_site)~int_ed_site1$def_mean)

#plot(int_ed_site1$mean_ED_site~log(int_ed_site1$area_ha))
plot(log(int_ed_site1$sum_ED_site)~log(int_ed_site1$area_ha),
...
### simple models
m1<-lm(mean_ED_site~def_mean, data = int_ed_site1)
summary(m1)

m2<-lm(sum_ED_site~def_mean, data = int_ed_site1)
summary(m2)

m3<-lm(sum_ED_site~def_sum, data = int_ed_site1)
summary.lm(m3)
...
results_ses_ed_int<-lm(ses.ed_int~(int_ed_site1$def_sum))
summary.lm(results_ses_ed_int)
par(mfrow=c(1,1))
plot((ses.ed_int)~(int_ed_site1$def_sum), 
     main="t= -4.289, p< 0.001",cex.main=.8,
...
#######
m1<-lm(int_commu2$mean_birds_ed~int_ed_site1$def_sum)
...
#df<-int_commu2[,c("site","interaction", "int_ed")]
#matrix_int_ed<-xtabs(int_ed~site+interaction, data=df)
...
ed<-join(int_commu2,k_int,by = "interaction", type = "left", match = "first")
...
```{r asymmetries}
#ed<-int_commu2
ed1<-subset(ed,!duplicated(ed[,1])) #### testing with unique interactions

##### testing whether interactions are asymmetric
...
####### Chi-squared to test differences 
type<-NULL
type_asym<-ifelse(ed$asym > 0,"positive","negative")
ed["type_asym"]<-type_asym
pos_asym<-as.vector(ed$asym[which(ed$asym > 0)])
neg_asym<-as.vector(ed$asym[which(ed$asym < 0)])
########### Chi-squared test
mytab <- with(ed,table(type_asym,asym))
ttest<-chisq.test(mytab)
(ttest)

######## testing with unique interactions 2669
asym<-NULL
for (i in 1:nrow(ed1)){
 asym [i]<- (ed1$mean_birds_ed[i]-ed1$mean_plants_ed[i])/max(ed1$mean_birds_ed[i],ed1$mean_plants_ed[i])
}
ed1["asym"]<-asym
ed1$asym
...
asym_scl<-NULL
for (i in 1:nrow(ed1)){
 asym_scl [i]<- (ed1$mean_birds_ed_scl[i]-ed1$mean_plants_ed_scl[i])/max(ed1$mean_birds_ed_scl[i],ed1$mean_plants_ed_scl[i])
}
ed1["asym_scl"]<-asym_scl
ed1$asym_scl


####### Chi-squared to test differences 
type<-NULL
type_asym<-ifelse(ed1$asym > 0,"positive","negative")
ed1["type_asym"]<-type_asym
pos_asym<-as.vector(ed1$asym[which(ed1$asym > 0)])
neg_asym<-as.vector(ed1$asym[which(ed1$asym < 0)])
########### Chi-squared test
mytab <- with(ed1,table(type_asym,asym))
ttest<-chisq.test(mytab)
(ttest)
...
#### scaled
type_scl<-NULL
type_asym_scl<-ifelse(ed1$asym_scl > 0,"positive","negative")
ed1["type_asym_scl"]<-type_asym_scl
#write.csv(ed, "ed_traits_new.csv")
########### Chi-squared 
mytab_scl <- with(ed1,table(type_asym_scl,asym_scl))
ttest_scl<-chisq.test(mytab_scl)
(ttest_scl)


########## Asymmetries and degree to test whether more asymetric interactions are related to more persisting interactions
mtest<-lm(degree~asym, data=ed)
summary(mtest)

mk3<-glmer(degree~asym + (1|Frugivore_Species)+(1|Plant_Species),data=ed,family="poisson")
summary(mk3)
str(ed)
...
mk4<-glmer(degree~mean_birds_ed_scl + mean_plants_ed_scl + (1|Frugivore_Species),data=ed1,family="poisson")
summary(mk4)

mk5<-glmer(degree~mean_plants_ed_scl + (1|Plant_Species),data=ed1,family="poisson")
summary(mk5)
...
#A= mean_birds_ed
#B= mean_plants_ed
 ### the difference in ED relative to combined ED of the interacting
### contour plot 

#propdif<-NULL
#for (i in 1:nrow(ed)){
 #propdif[i]<- (ed$mean_birds_ed[i]-ed$mean_plants_ed[i])/
  #            (ed$mean_birds_ed[i]+ed$mean_plants_ed[i])
#}
#ed["prop_dif"]<-propdif
#ed$prop_dif
#cor.test(ed$asym,ed$prop_dif)
#write.csv(ed, "ed_traits_new.csv")

#plot(ed$prop_dif~ed$degree,ylab="Proportional difference in asymmetry of Evolut distinct",
 #    xlab="Number of fragments in which the interaction occurs",
  #   main = "z= -2.084, p = 0.037")
#dev.copy2pdf(file="Fig. k_int vs asymmetry.pdf")
...
library(plotly)
  x = log(ed$mean_plants_ed)
  y = log(ed$mean_birds_ed)

####
cols<-c("blue", "yellow")
q<-ggplot(data=ed, aes(x=x, y=y, z=asym)) +
...
geom_density2d(size=1)+
  ...
  geom_point(aes(colour = asym), shape = 21, size = 1, stroke = 0.2)+
...
dev.copy2pdf(file="Fig. asymmetries.pdf")
```
