...
data<-data[!data$Frugivore_Species2==("Estrilda_astrild"),]
...
bt100<-lapply(bt100,drop.tip,tip="Estrilda_astrild")
...
#### ------------------- BIRDS - 186 sp among the selected communities - 223 regional level
...
df1<-sd_ED_commu[with(sd_unique_commu2,order(interaction)),]
df1<-df1[,c(1:8,109)]
...
int_commu1<-int_commu1[,c(1:4,10,112)]
...
library(plotly)
  x = (ed1$mean_plants_ed_scl)
  y = (ed1$mean_birds_ed_scl)

####
cols<-c("blue", "yellow")
q<-ggplot(data=ed1, aes(x=x, y=y, z=asym_scl)) +
